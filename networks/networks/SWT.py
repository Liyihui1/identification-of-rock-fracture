...
if __name__ == '__main__':
    ...
    output = model(x)
    print(x.shape)
    # def downsample(input_resolution, dim=32, norm_layer=nn.LayerNorm):
    ...
    # embed = PatchEmbed(
    #         img_size=256, patch_size=1, in_chans=32, embed_dim=32,
    #         norm_layer=nn.LayerNorm)
    # # unembed = PatchUnEmbed(img_size=256, patch_size=1, embed_dim=32)
    # pd = nn.Dropout(p=0)
    # model = BasicLayer(dim=32, depth=3, num_heads=8, input_resolution=(embed.patches_resolution[0], embed.patches_resolution[1])
